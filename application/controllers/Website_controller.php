...
class Website_controller extends CI_Controller {
	...
	public function website_home()
	{
		...
		$data['banner_offer'] = $this->db->query('SELECT * FROM `tbl_products` where pro_off_percentage > 4  ORDER BY `update_date` DESC limit 9')->result();
		...
		if($role =='customer')
		{
			...
			if(!empty($data['cart_data']))
			{
				foreach ($data['cart_data'] as $key => $value)
				{
					...
					$data['stock_list'] = $this->crud_model->selectquery_array('tbl_stock_list',$datavalues21);
					if(!empty($data['stock_list']))
					{
						$datavalues21 = array('product_id' => $value->product_id);
						$balance = $data['stock_list'][0]->current_available - $value->quantity;
						$datavalues22 = array('current_available' => $balance);
						$this->crud_model->update('tbl_stock_list', $datavalues22, 'stock_id', $data['stock_list'][0]->stock_id);
					}
					//stock reduce end
